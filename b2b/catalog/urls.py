...
urlpatterns = [
    ...
    # Со слэшем и без — иначе PATCH/PUT без слэша даёт 500 (APPEND_SLASH + тело запроса).
    path('products/<uuid:pk>/', ProductRetrieveUpdateAPIView.as_view(), name='product-detail'),
    path('products/<uuid:pk>', ProductRetrieveUpdateAPIView.as_view(), name='product-detail-no-slash'),
